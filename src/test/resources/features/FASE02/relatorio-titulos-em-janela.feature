#language: pt
Funcionalidade: Tela de relatório de Titulos em Janela
    Eu como analista de testes
    Quero acessar a aplicação cineflix da recordTv
    Para testar a tela de relatório de titulos em janela do sistema

    Contexto: 
    Dado que eu acesse a aplicacao cineflix
    E realizar o login na aplicacao com sucesso
    Quando clicar no menu "Relatório"
    E clicar na opção "Títulos em Janela"

    @automacao
    Cenário: Validar elementos da tela de relatório de Títulos em Janela
        Então o sistema deve exibir o titulo "Título em Janela" 
        E o sistema deve exibir a tela de relatório "Títulos em Janela" contendo os seguintes filtros:
        | Período de Janela |
    ...
    Cenário: Validar funcionamento da caixa com total de itens encontrados
        # Quando eu realizar o filtro na tela de relatório de Títulos em Janela
